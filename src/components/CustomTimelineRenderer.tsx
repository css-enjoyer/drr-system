...
function CustomTimelineRenderer({ branchId }: { branchId: string }) {
  ...
  const CustomEditor = ({ scheduler }: CustomEditorProps) => {
    ...
    const handleChange = (value: string | DurationOption, name: string) => {
      ...
      >
        ...
        >
          ...
          <Typography variant="h4">
            {event ? "Edit" : "Reserve"} Room {scheduler.state.room_id.value}
          </Typography>
          <TextField
          ...
          />
          <TimePicker label="Start time" value={formState.start} readOnly />
          <TimePicker label="End time" value={formState.end} readOnly />
          ...
          <TextField
            ...
            helperText={
              formState.pax < 4 || formState.pax > 12
                ? "Pax should be 4-12"
                : ""
            }
          ...
          <TextField
            ...
            helperText={
              formState.purp.length > 100
                ? "Limited to 100 characters only"
                : ""
            }
  ...
  return (
    <Scheduler
          ...
          >
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: "7px" }}>
              ...
              <Typography variant="caption">
                ...
                {
                  eventsState.find(
                    (eventState) => eventState.event_id === event.event_id
                  )?.stuRep
                }
              </Typography>
            </Box>
            ...
            >
              <Button
                size="small"
                onClick={() =>
                  updateEventTitle(event.event_id + "", "Unavailable")
                }
              >
                Set as Unavailable
              </Button>
              <Button
                size="small"
                onClick={() =>
                  updateEventTitle(event.event_id + "", "Departed")
                }
              >
                Confirm Departure
              </Button>
      ...
      deletable={
        isAdmin(authContext?.user?.email, admins) ||
